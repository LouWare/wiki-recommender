...
function tokenize(text, lang = "de") {
  ...
  const sw = lang === "de" ? STOPWORDS_DE : lang === "en" ? STOPWORDS_EN : new Set();
...
function mapSum(a,b,wa=1,wb=1){const out=new Map(a); for(const [k,v] of b) out.set(k,(out.get(k)||0)+wb*v); if(wa!==1) for(const[k,v]of a) out.set(k,(out.get(k)||0)+(wa-1)*(a.get(k)||0)); return out;}
...
function sanitizeWikiHtml(html){ if(!html) return ""; const fixProto = (s)=> s.replace(/srcset=\"\s*\/\//g,'srcset="https://').replace(/src=\"\s*\/\//g,'src="https://'); let out=fixProto(html); out=out.replace(/<a\s+([^>]*?)href=\"([^\"]+)\"/g,(m,pre,href)=>`<a ${pre} href="${href}" target="_blank" rel="noopener noreferrer"`); return out; }

function slugify(s){return s.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-_]/g,'').slice(0,80);}
...
export default function WikiRecommenderAppV42(){
  ...
  async function runSearch(q){ setLoading(true); setError(""); try{ const titles=await searchTitles(q,lang); const base=await Promise.all(titles.slice(0,15).map(async t=>{ try{ const s=await fetchSummary(t.title,lang); return { id:s.pageid||t.id, title:s.title||t.title, summary:s.extract||"", excerpt:t.excerpt||s.description||"", thumbnail:s.thumbnail, url:s.url||`https://${lang}.wikipedia.org/wiki/${encodeURIComponent(t.title)}` }; }catch{ return { id:t.id, title:t.title, excerpt:t.excerpt||"", url:`https://${lang}.wikipedia.org/wiki/${encodeURIComponent(t.title)}` }; }})); const relatedBlocks=await Promise.all(base.slice(0,3).map(b=>fetchRelated(b.title,lang,20))); const related=relatedBlocks.flat(); const byTitle=new Map(); for(const a of [...base,...related]) if(a?.title && !byTitle.has(a.title)) byTitle.set(a.title,a); const merged=Array.from(byTitle.values()); setCandidates(merged); }catch(e){ setError(e?.message||"Netzwerkfehler"); } finally{ setLoading(false); }}

  function updateProfilesFromArticle(a){ const text=`${a.title} ${a.excerpt||""} ${a.summary||""}`; const vec=termFreq(tokenize(text,lang)); const decayLT=0.98, decayST=0.82; const upd=(mapObj,decay)=>{ const m=new Map(Object.entries(mapObj)); for(const[k,v]of m) m.set(k,v*decay); for(const[k,v]of vec) m.set(k,(m.get(k)||0)+v); return Object.fromEntries(m); }; setProfileLT(prev=>upd(prev,decayLT)); setProfileST(prev=>upd(prev,decayST)); const newHist=[{title:a.title,lang,ts:Date.now()},...history].slice(0,200); setHistory(newHist); }
  ...
  const closeArticle=()=>{ if(active){ updateProfilesFromArticle(active); fetchRelated(active.title,lang,20).then(rel=>{ const map=new Map(candidates.map(c=>[c.title,c])); for(const r of rel) if(!map.has(r.title)) map.set(r.title,r); setCandidates(Array.from(map.values())); }).catch(()=>{}); } setModalOpen(false); setActive(null); setFullHtml(""); setToc([]); };
